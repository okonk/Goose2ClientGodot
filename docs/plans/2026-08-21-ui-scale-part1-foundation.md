...
`UiScale` is a small non-static class (no Godot usings): `float Factor`, `float Factor(float raw)`, `int AutoFactor(int windowHeightPx)`, `int ScaleSize(float basePx)`, `Vector2I ScaleSizeI(Vector2I v)`, plus **pinned constants** `public const float MinFactor = 1f, MaxFactor = 3f, Step = 0.5f` (the slider range; `Factor` clamps/snaps to these). `Vector2I` is fine in the test project (GodotSharp is already referenced there, see `WindowPlacementTests`).
...
1. `Scale.Factor(factor)` (normalizes; NaN-safe per Task 1). **The first `Apply` call always runs** (the startup apply). Every subsequent call **early-returns when the normalized factor is unchanged**, whatever the reason (window-resize spam costs one compare).
...
```
`GeomRecord` (`internal` — referenced from `BaseWindow`'s `_geom` field, so NOT private/nested-private): `(Control c, float oLeft, float oTop, float oRight, float oBottom, Vector2 minSize /* zero if none */, (StringName,int)[] constants /* empty if none */)`. **Offsets are stored as the recorded raw floats.** **Offsets, not Position/Size** — offsets are anchor-relative by construction, so the same record works for top-left-anchored controls (where offsets ≡ position/size) and for anchored roots (`ChatWindow` bottom-left, `Toolbar` right-edge) where scaling `Position` would detach the edge-stick.
...
2. **2× apply:** `UiScaleApplier.Instance.Apply(2f, ApplyReason.UserCommit)`. Then assert:
   ...
   - Sampled geometry: `Vitals` root `Size == (366, 110)` (tscn 183×55) and `Position == (16, 16)` (tscn 8,8); one `ItemSlot` under Inventory has `CustomMinimumSize == (64, 64)`; `ChatWindow` root offsets `OffsetTop == −426` and `OffsetBottom == −10` (tscn −213/−5 doubled — edge-stick preserved by offset scaling, NO reposition involved; ChatWindow is not a `BaseWindow`); a `BaseWindow`-derived dialog still satisfies `WindowPlacement` containment: `0 <= X <= Max(0, canvas.X - w.Size.X)` and `0 <= Y <= Max(0, canvas.Y - w.Size.Y)` (the `Max(0, …)` matters at the tiny headless canvas: a 366-px-wide window against a 100-px canvas gives a negative bound — the real `Resolve` clamps to `0`, the assertion must match its postcondition, not fail on it).
