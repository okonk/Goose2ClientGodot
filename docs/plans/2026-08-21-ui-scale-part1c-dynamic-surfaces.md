...
**Spec:**
- `MultiWindowMetrics` (pure; pattern: `TooltipMetrics`): `RowPitch(factor)` → float — EXACTLY the literal `11.18f` at factor 1 (identity guarantee, no rounding), else `11.18f × factor` rounded half-away-from-zero; `LineFont(factor)` = `UiScale.ScaleSize(10)`, `ButtonFont(factor)` = `ScaleSize(12)`. xUnit: the 1× row equals today's literals (11.18f / 10 / 12); 1.5×, 2×; round-trip.
- `_Ready`: create the labels using `MultiWindowMetrics` at `UiScaleApplier.Instance.CurrentFactor` (native scaling for windows spawned late); give EACH line label the Part 1B Task 2 skip-meta (its geometry is owned by the metrics + `Relayout`, not the generic snapshot — the snapshot would rescale the already-scaled offsets and double-scale at 2×); register the button fonts through `applier.ApplyFontSize`; `ScaleRegister()` at end of `_Ready` (the tscn `GenericInfo` frame offsets scale generically; line labels are skipped).
- `Relayout()` (the `IScalableWindow` contract, Part 1B Task 1): reposition the `_lines` at `RowPitch(factor)` from `LinesOrigin` and nothing else (fonts are the apply pass's registry step). Runs on HIDDEN windows too (cheap — `LineCount` labels), so the first show after a commit is already correct.
...
2c. **Runtime multi-window leg (SC-11, Task 4):** with the HUD at 2×, instantiate `res://Scenes/UI/InfoWindow.tscn`, `UiLayer.AddChild`, await a frame; drive it with `new MakeWindowPacket()` (public fields — set the same values the builder at `Scripts/Network/Packets/MakeWindowPacket.cs:21` sets) via its internal `OnMakeWindow`, plus one `OnWindowLine`; assert two adjacent line labels' `Position.Y` differ by `MultiWindowMetrics.RowPitch(2f)` and `GetThemeFontSize("font_size") == 20`. `Apply(1f)` → pitch back to exactly 11.18f. `QueueFree` + await + registration gone (same cleanup rule as 2b).
...
**Explicit automated gap (SC-16, stated per review):** the LOADING overlay is NOT covered by this self-test — headless can never trigger a map transition, so the loading scene's scaled geometry is covered ONLY by Part 2's manual M8 leg. No headless assertion exists or can exist for it; M8 is the sole gate.
...
| Invariant | Proved by |
|-----------|-----------|
| 1× is bit-identical to today (no visual regression at default) — SC-05 | Task 5 steps 1 & 3 |
