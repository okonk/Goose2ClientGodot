...
- `GameManager` builds the HUD at `Scripts/GameManager.cs:327` (`UiLayer.AddChild(Hud)`) — the initial `Apply` must precede this line.
...
- GodotSharp 4.7.1, verified by **reflection on the actual project DLL** (`/opt/Godot_v4.7.1-stable_mono_linux_x86_64/GodotSharp/Api/Release/GodotSharp.dll`), not docs:
  ...
  - `Control.GetThemeFontSize(StringName, StringName)` / `GetThemeConstant(...)` — effective values (used for snapshot division).
  ...
  - `Control.Position`/`Size` setters write the anchor-relative offsets.
- `theme_override_font_sizes` occurrences to migrate: `Scenes/UI/BankWindow.tscn:43` (9), `Scenes/UI/ChatWindow.tscn:47` (`normal_font_size` 12, RichTextLabel), `Scenes/UI/ChatWindow.tscn:55` (12), `Scenes/UI/DebugWindow.tscn:23,32` (12), `Scenes/UI/VendorWindow.tscn:43` (10). Raw C# overrides: `Scripts/UI/BaseMultipleWindow.cs:58` (12) and `:68` (10). (Bridge files `ChatBubble`/`BattleTextLine`/`BridgedNameLabel` are world-space — **do not touch**.)
...
- R1: per-window hand-written layout constants → **generic snapshot** (`UiScaleLayout`), base recovered by dividing end-of-`_Ready` geometry by the current factor.
...
- `RoundTrip_DivideRecoversBase` (supports Task 4's divide-by-factor snapshot): for factor `1.5f` and every integer base `b ∈ 1..64`: `int(round((float)(int)(round(b × 1.5)) / 1.5f)) == b`. If any `b` fails, the snapshot premise is wrong — stop and re-derive; do not weaken the test.
...
| Snapshot divide-by-factor is lossless (R1 premise) | `RoundTrip_DivideRecoversBase` (adversarial: fails on any wrong rounding policy) |
...
- `Resolve_WindowLargerThanCanvas_ClampsToOrigin`: size `(2000, 1500)`, current `1280×720` → `(0, 0)`.
- `Resolve_TitleBarAllowance_Scaled`: saved y such that window bottom exceeds canvas: `currentCanvas (1280,720)`, window height `700`, saved `(100, 600)` → default allowance clamps y to `720−24=696`… assert y `== 696`; with `titleBarAllowance: 48` assert y `== 672`.
...
- Modify: `Scripts/GameManager.cs` (create/`Instance` in `_Ready` before `UiLayer.AddChild(Hud)` at line 327; initial `Apply` there too)
...
- Creation order: window builds nodes → snapshots geometry (Task 5) → `RegisterWindow(this)` → first `Relayout()` is NOT called by registration (build-time geometry is already at the current factor — registration is bookkeeping only).
...
**GameManager wiring:** in `_Ready`, after `CharacterSettings` exists and before `UiLayer.AddChild(Hud)` (`Scripts/GameManager.cs:327`):
```csharp
...
### Task 4: `UiScaleLayout` generic snapshot (R1 mechanism)
...
public static class UiScaleLayout
{
    ...
    public static List<GeomRecord> Snapshot(Control root, float factor);
    public static void Apply(List<GeomRecord> records, float factor);
}
```
`GeomRecord` (nested or private): `(Control c, Vector2 pos, Vector2 size, Vector2 minSize /* zero if none */, (StringName,int)[] constants /* empty if none */)`.

**`Snapshot(root, factor)` contract:**
- Preconditions: called exactly once, at END of the window's `_Ready`, after ALL build-time C# geometry (which must have used `UiScaleApplier.Instance.ScaleSize` for any code-side pixels). Reads the current factor's geometry and divides: `base = RoundEvenHalf(current / factor)` — lossless by Task 1's `RoundTrip_DivideRecoversBase`.
- Walks `root` + all descendants (depth-first). Skips any node (and its subtree) with meta `ui_scale_skip == true` (R: per-frame/dynamic controls opt out). For each `Control`: record `Position`, `Size`, `CustomMinimumSize` (skip if `Vector2.Zero`), and effective theme constants via `GetThemeConstant(name)` — snapshot ONLY names returned by a fixed list: `"separation"`, `"h_separation"`, `"v_separation"` (the only ones used in `Scenes/UI/*.tscn` — verify with `grep -rn theme_override_constants Scenes/UI` at implementation time; extend the list only with cited occurrences). Skip constants whose value is `0`.
- Postconditions: `Apply(records, factor)` reproduces exactly the end-of-`_Ready` geometry when `factor` is unchanged; anchors are never recorded or touched.

**`Apply(records, factor)`:** for each record, if `c.IsInsideTree()`: `c.Position = scale(c.Pos)`; `c.Size = scale(c.Size)` (per-axis `int(round(base×f))`); min-size via `c.CustomMinimumSize`; constants via `c.AddThemeConstantOverride(name, scaled)`. **This file contains NO Godot-free math of its own** — it takes the factor as `float` and does explicit `MathF.Round(x, AwayFromZero)` on plain floats (mirroring Task 1's policy; `UiScale`'s instance isn't reachable from a static helper without a parameter, and passing the factor is the seam).

**Flags:** `Position`/`Size` setters write the anchor-relative offsets (`GodotSharp Control`), so anchor-attached controls (e.g. `CloseButton` at `offset_left = -18` in `BaseWindow.tscn`) scale their offsets correctly with anchors untouched.

**Proof:** no direct unit test (needs a live tree) — proven by Task 8: (a) 1× no-op run (snapshot→apply at factor 1 must leave every rect bit-identical), (b) factor-2 audit (every registered window rect equals `round(base×2)` for a sampled set incl. `VitalsWindow` root 183×55→366×110 and an `ItemSlot` min-size 32→64).
...
- `_Ready()`: at the VERY END (after drag-handle wiring, `MoveChild` etc. — after `BaseWindow.cs:74`'s `MoveChild` call):
  ```csharp
  var applier = UiScaleApplier.Instance;
  _geom = UiScaleLayout.Snapshot(this, applier.Factor);
  applier.RegisterWindow(this);
  ```
  Connect `TreeExited += () => applier.UnregisterWindow(this);` in the same place.
  **Invariant:** no code after these lines may set geometry on `this` or descendants without going through `applier.ScaleSize` (the snapshot is the base from now on).
...
**`BaseMultipleWindow` (runtime line labels + fonts):**
- `_Ready` line creation (lines 62-72): `label.AddThemeFontSizeOverride("font_size", LineFontSize)` → `UiScaleApplier.Instance.ApplyFontSize(label, LineFontSize)`; `label.Position = LinesOrigin + new Vector2(0, i * LineRowHeight)` → `label.Position = LinesOrigin * f + new Vector2(0, i * (LineRowHeight * f))` via a private `RelayoutLines()` helper using `applier.ScaleSize` — called at build AND from an overridden `Relayout()` (call `base.Relayout()` first, then `RelayoutLines()`). `LinesOrigin` (6,22) and `LineRowHeight` (11.18) stay as the base constants.
- Button font (line 58): `b.AddThemeFontSizeOverride("font_size", ButtonFontSize)` → `applier.ApplyFontSize(b, ButtonFontSize)`.

**Other `BaseWindow` subclasses:** no changes expected — their geometry is `.tscn`/generic. If any subclass sets geometry in `_Ready` without `ScaleSize`, the 1× no-op self-test (Task 8) still passes (factor 1 divides by 1) but the 2× audit will catch it — treat audit failures as "fix this window's build code to use `ScaleSize`", not as scaler bugs.
...
### Task 6: non-`BaseWindow` roots (Vitals, Toolbar, TooltipManager)

**Files:**
- Modify: `Scripts/UI/VitalsWindow.cs:33` (`_Ready`)
- Modify: `Scripts/UI/Toolbar.tscn`'s script (locate: `grep -l "class Toolbar" Scripts` — the Toolbar control script)
...
- `Toolbar`: registers (tscn offsets scale; its Options button wiring is untouched).
...
**Files:**
- Modify: `Scripts/GameManager.cs` (`_Ready`, after HUD build and settings load): read `OS.GetCmdlineUserArgs()`; if it contains `"+selftest=ui_scale"`, run the sequence below on the NEXT frame (one `ProcessFrame` await) and `GetTree().Quit(failed ? 1 : 0)`.
...
1. **1× no-op:** for each registered `BaseWindow`: snapshot its descendants' `(Position, Size)` into a dict, call `w.Relayout()`, re-read; assert bit-identical (factor is 1 from startup Apply). Catches R1 snapshot bugs and any `_Ready` code that sets geometry after the snapshot point.
2. **2× apply:** `UiScaleApplier.Instance.Apply(2f, ApplyReason.UserCommit)`. Then assert:
   ...
   - Sampled geometry: `Vitals` root `Size == (366, 110)` (tscn 183×55); one `ItemSlot` under Inventory has `CustomMinimumSize == (64, 64)`; Chat window's right/bottom edge offset (edge-stuck left-bottom per `DefaultWindowLayout`/placement) — assert its `Position.X == 0` (left edge preserved) — the placement re-solve worked through `RepositionForCurrentCanvas`.
   - `BaseWindow`-derived dialog windows still satisfy `WindowPlacement` containment: `0 <= X <= canvas.X - w.Size.X` etc.
...
**Red/green:** run before Task 5/6 land → expected FAIL (no registered windows / no `IScalableWindow`); after Task 7 → PASS. Final state of this task is the gate for the whole part: `bash tools/tests/run_ui_scale.sh` exits 0 with labeled `OK` lines, no `ERR_`/script-error output.
...
| Snapshot divide-by-factor is lossless (R1) | Task 1 `RoundTrip_DivideRecoversBase` + Task 8 step 3 (in-engine) |
