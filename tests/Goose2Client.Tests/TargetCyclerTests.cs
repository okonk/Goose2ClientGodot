...
public class TargetCyclerTests
{
    ...
    [Fact] public void NpcTargetTypeIncludesPlayerAndPetWhenPvpEnabled()
    {
        ...
        Assert.Equal(2, TargetCycler.Next(all, C(1,5,5,CharacterType.Monster), (5,5), 100, SpellTargetType.NPC, true, true)?.LoginId);
        Assert.Equal(3, TargetCycler.Next(all, C(2,6,5,CharacterType.Player), (5,5), 100, SpellTargetType.NPC, true, true)?.LoginId);
    }
    ...
    [Fact] public void NoFilteringForNpcPlayerTypeWhenPvpEnabled()
    {
        CurrentMapFlags.Value = new MapFlags(true, true, true);
        var all = new List<TargetCandidate>{ C(1,5,5,CharacterType.Player), C(2,6,5,CharacterType.Monster) };
