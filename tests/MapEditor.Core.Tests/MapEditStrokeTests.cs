...
public class MapEditStrokeTests
{
    ...
    [Fact]
    public void BlockedToggle_LoopTogglesEachVisitedCellExactlyOnce()
    {
        var doc = MapDocument.Create(10, 10);
        var session = new MapEditSession(doc);

        session.BeginStroke(MapEditTool.BlockedToggle, 0, 0);
        session.ContinueStroke(9, 0);
        session.ContinueStroke(9, 9);
        ...
        for (var y = 0; y < 10; y++)
        {
            for (var x = 0; x < 10; x++)
            {
                bool onBoundary = x == 0 || x == 9 || y == 0 || y == 9;
                Assert.Equal(onBoundary ? MapDocument.BlockedFlag : 0, doc[x, y].Flags);
            }
        }
    }

    [Fact]
    public void BlockedToggle_UndoRedoAndCancelReplayExactFlags()
    {
        var doc = MapDocument.Create(10, 10);
        int[] before = { 1, unchecked((int)0x80000001), int.MinValue | 3, 0b101 };
        for (var x = 0; x < 4; x++)
        {
            doc.SetFlags(x, 0, before[x]);
        }
        var session = new MapEditSession(doc);
        ...
        for (var x = 0; x < 4; x++)
        {
            Assert.Equal(before[x] ^ MapDocument.BlockedFlag, doc[x, 0].Flags);
        }

        Assert.True(session.Undo());
        for (var x = 0; x < 4; x++)
        {
            Assert.Equal(before[x], doc[x, 0].Flags);
        }
