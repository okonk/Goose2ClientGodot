...
**Step ordering / tests:** this task has no pure-xUnit surface (Godot types). Proof comes from Part 1C Task 5's self-test (audit + smoke) and Task 3/9's 1× no-op run. Compile + existing test suite green is the gate here.
...
- `public void Relayout()` — geometry ONLY (placement is a separate step; the applier calls it then `RepositionFromSaved`, Task 1 step 6/7):
  ```csharp
