...
public static class OptionListMetrics
{
    ...
    private const int IconLinePaddingY = 2;
    private const int IconLineHeight = IconSize + IconLinePaddingY * 2;
    // Tighter row pitch for icon-less lists (text only), matching the pre-icon spacing.
    private const int TextLineHeight = 23;
    ...
    public static int LineHeight(bool anyIcon) => anyIcon ? IconLineHeight : TextLineHeight;
    ...
    public static Vector2 LinePosition(int index, float factor, bool hasHeading, bool anyIcon)
    {
        var originY = LinesOriginY + (hasHeading ? HeadingHeight : 0f);
        var basePos = new Vector2(LinesOriginX, originY + index * LineHeight(anyIcon));
        if (factor == 1f)
            return basePos;
        return new Vector2(
    ...
    public static Vector2 LineSize(float factor, bool anyIcon)
        => new(UiScale.ScaleSize(LinesWidth, factor), UiScale.ScaleSize(LineHeight(anyIcon), factor));
    ...
    public static int WindowHeight(int lineCount, float factor, bool hasHeading, bool bottomButtons, bool anyIcon)
    {
        var h = LinesOriginY + (hasHeading ? HeadingHeight : 0f) + lineCount * LineHeight(anyIcon) + BottomMargin;
        if (bottomButtons)
            h += ButtonRowHeight;
