...
- Modify: `Scripts/CharacterSettings.cs` — (a) the window-settings record (returned by `GetWindowSettings`, `CharacterSettings.cs:144`) gains ADDITIVE **non-nullable** fields `Size` (Vector2 — display px at save time) and `Factor` (float): value types default to **ZERO** when the key is absent (not null), and the serializer emits the declared PascalCase member names — JSON keys `"Size"`/`"Factor"` per window section (verify against the existing JSON fixtures). Task 5 falls back when `Size == default` / `Factor <= 0`. (b) NEW method `SetWindowVisible(string windowName, bool visible)` — a visibility-ONLY update: writes `Visible` and leaves `Position`/`Size`/`Factor`/`CanvasSize` untouched (the toggle/close path must not corrupt the quad — review finding: the existing toggle saves live Position + CanvasSize without Size/Factor, producing mixed-coordinate quads after scaling and breaking first-time close at 2×). The full quad is persisted ONLY by the drag-end `SetWindowSetting` call (all four fields atomically).
- Test: `tests/Goose2Client.Tests/WindowPlacementTests.cs` (extend)
- Test: `tests/Goose2Client.Tests/CharacterSettingsJsonTests.cs` (extend — the schema change lands here): `WindowSettings_SizeFactor_RoundTrip` (both fields persist and reload); `WindowSettings_LegacyJsonWithoutSizeFactor` (a pre-feature JSON string deserializes with `Size == default, Factor == 0`, nothing else disturbed); `SetWindowVisible_PreservesFullQuad` (save full quad → `SetWindowVisible(false)` → reload → all four quad fields byte-identical, `Visible` flipped); `SetWindowSetting_DragSave_UpdatesAllFourAtomically` (one call updates Position/Size/Factor/CanvasSize together, `Visible` preserved).
...
C720 = the 1280×720 canvas. Every expected value below is derived from the branch structure above (left/right offsets in SAVED space, band ≤ `TitleBarHeight` = 24, equidistant-keep, then containment clamp) — recompute by hand before implementing; a test whose arithmetic doesn't follow the spec rejects the spec itself (review finding).
...
- `ResolveScaled_HotbarCommitAt2x_ClampsMarginToEdge` (real hotbar quad `(520, 679)`, 351×36, f1, C720; at `(702, 72), factor 2`, C720): x — left 520, right `1280 − (520+351) == 409` (> 24, not band; equidistant: 409 < 520 → trailing) → `1280 − 702 − Round(409×2) == 578 − 818 == −240` → clamped to **`0`** (scaled margin 818 doesn't fit the 578 available — the documented clamp); y — bottom `720 − (679+36) == 5` → band → **kept 679** (within clamp 696) → **`(0, 679)`**.
- `ResolveScaled_ScaleCommitRoundTrips` (the F1 trace, pinned): the SAME invariant quad: `@1×` `(351,36)` → x `1280 − 351 − 409 == 520`, y band-kept 679 → **exactly `(520, 679)`**; `@1.5×` `(527, 54)`, allowance `36` → x `1280 − 527 − Round(409×1.5 == 613.5 → 614) == 139`, y 679 → `(139, 679)`; `@2×` → `(0, 679)` (above). The quad never changes across commits, so deriving at the saved factor returns the saved position EXACTLY — a capture-only or stale-position model cannot satisfy this.
- `ResolveScaled_DragAtScale_CommitAndRoundTrips`: quad `((800, 600), (400, 72), 2, C720)` (a 400-wide window dragged at 2×): commit to 1× `(200, 36)` — x: left 800, right `1280 − 1200 == 80` → trailing (80 < 800) → `1280 − 200 − Round(80×0.5) == 1040`; y: bottom `720 − 672 == 48` → trailing (48 < 600) → `720 − 36 − 24 == 660` → **`(1040, 660)`**; back at 2× `(400, 72)` → x `1280 − 400 − 80 == 800`, y `720 − 72 − 48 == 600` → **exactly `(800, 600)`**.
- `ResolveScaled_MarginClampsWhenScaledWindowExceedsCanvas`: quad `((100, 679), (351, 36), 1, C720)`, `(702, 72) @ 2`, C720, allowance `48`: x — left 100, right 829, equidistant-keep (829 > 100) → **100**; y — band-kept 679 → clamped to `720 − 48 == 672` (title bar stays reachable) → **`(100, 672)`**; on a `1280×100` canvas → y clamped to `100 − 48 == 52` → `(100, 52)`.
- `ResolveScaled_TitleBarAllowance_Scaled`: quad `((100,700),(100,100),1,C720)`, `(100,100)@1`: y — bottom `720 − 800 == −80` → trailing → `720 − 100 − Round(−80) == 700` → clamped → y `696` (allowance 24) / `672` (allowance 48); x kept 100.
...
```
`Instance` has an `internal set` (set by `GameManager._Ready`, same assembly — NOT `private set`, which GameManager can't call). `Theme` is `GD.Load<Theme>("res://Assets/UI/GameTheme.tres")` — C# has NO `Preload<T>`; `GD.Load` is the API. `_fonts` is FLAT with ownership by ancestry at unregister time (NOT by registration — review finding: `ApplyFontSize` runs during the window's `_Ready`, BEFORE its own registration, so "the most recent registration" would be the PREVIOUS window). ``RegisterWindow` is bookkeeping ONLY and throws `ArgumentException` for a non-`Control` `IScalableWindow` (a stored null `ControlRef` would break the dictionary — a non-`Control` caller is a bug that must surface at the call site, not be silently skipped); it returns the new `WindowRegistration`, or the EXISTING one for an already-registered window (idempotent — the `CallDeferred` fallback and a direct call can both fire). The initial Relayout is owned by the window's `ScaleRegister` (Task 5), not here. The read-only seams (`RegisteredWindows`, `TryGetFontBase`, `Theme`) are public for the Task 10 self-test — the applier surface is already public, and this repo has no InternalsVisibleTo infra.
...
- `RepositionForCurrentCanvas()` (lines 90-109) is REPLACED by `RepositionFromSaved()` (parameterless — the quad needs no arguments; this also dissolves the old draft's signature-vs-`_Ready` conflict):
  ...
  public void RepositionFromSaved()
  {
      ...
      var pos = ws != null && ws.Position != default ? ws.Position : DefaultWindowLayout.For(WindowName);
...
### Task 8: vitals portrait scaling (`VitalsPortraitMetrics`)
...
| Edge margins are logical px: they scale with the factor across a commit, and 1×→2×→1× round-trips EXACTLY (saved-quad model) | Task 2 `ResolveScaled_HotbarCommitAt2x_ClampsMarginToEdge` / `ResolveScaled_ScaleCommitRoundTrips` / `..._DragAtScale_CommitAndRoundTrips` / `..._MarginClampsWhenScaledWindowExceedsCanvas` |
