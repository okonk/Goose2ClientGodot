...
- `RepositionForCurrentCanvas()` (lines 90-109) is REPLACED by `RepositionFromSaved()` (parameterless — the quad needs no arguments; this also dissolves the old draft's signature-vs-`_Ready` conflict):
  ...
  public void RepositionFromSaved()
  {
      ...
      var savedSize = ws != null && ws.Size != default ? ws.Size : (DefaultWindowLayout.LegacySize(WindowName) ?? _tscnSize);
  ...
  ```
  Legacy settings (no `Size`/`Factor` keys — everything shipped to date): `savedSize = DefaultWindowLayout.LegacySize(WindowName) ?? _tscnSize`, `savedFactor = 1` — exactly the true saved pair (the `LegacySize` leg is non-null only for `Options`, whose tscn Part 2 grows 112→240 — every other window's legacy size IS its tscn size) (pre-feature saves were 1× tscn-size positions), so old files place identically to today. Callers: `ScaleRegister` (registration, post-relayout), the applier's placement step (every commit), and `GameManager`'s canvas-resize walk (`CollectBaseWindows` → `RepositionFromSaved` — the walk keeps its shape, one method renamed). Because the quad is invariant across commits, the walk, commits, and registration all compose: no previous-canvas tracking, no size capture (review findings on stale captures).
...
2. **2× apply:** `UiScaleApplier.Instance.Apply(2f, ApplyReason.UserCommit)`. Then assert:
   ...
   - **Party tile leg (Task 9):** one `PartyMember` under `PartyWindow/MemberList` has `CustomMinimumSize == (174, 66)` and its `NameText` `OffsetBottom == 22` (tscn 11 doubled).
   - **BuildStamp exception (Task 9):** the `BuildIdLabel` under the GameManager autoload's `BuildStampOverlay` CanvasLayer has `GetThemeFontSize("font_size") == 10` at 2× (the fixed local override beats the theme default's 20 — the design's unscaled-stamp guarantee).
...
| Edge margins are logical px: they scale with the factor across a commit, and 1×→2×→1× round-trips EXACTLY (saved-quad model) | Task 2 `ResolveScaled_HotbarCommitAt2x` / `ResolveScaled_ScaleCommitRoundTrips` / `..._DragAtScale_CommitAndRoundTrips` / `..._LeadingMarginScales` / `..._ClampWhenScaledWindowExceedsCanvas` |
