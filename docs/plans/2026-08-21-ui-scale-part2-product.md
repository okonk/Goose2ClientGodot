...
- `HSlider` (GodotSharp 4.7.1, same DLL): properties `MinValue`, `MaxValue`, `Step`, `Value`; signals `ValueChanged(float)` and `DragEnded`; `IsDragging` is NOT a property — drag state is tracked locally from `mouse_entered`+`Input.IsMouseButtonPressed` or, more simply, a `bool _dragging` set by the mouse-down/`DragEnded` pair (see Task 4 — the exact mechanism is pinned there by behavior, not API).
...
- **Commit rule:** track a local `_dragging` on the slider: `mouse_exited`/`mouse_entered` are NOT used — set `_dragging = true` on `InputEventMouseButton` left press via the slider's `GuiInput` (mirror `BaseWindow`'s pattern, `Scripts/UI/BaseWindow.cs:120-132`), set `_dragging = false` on `DragEnded`.
  - `ValueChanged(v)`: update `ScaleValueLabel` ALWAYS (live feedback while dragging). If `!_dragging` (keyboard arrow / programmatic) → `Commit(v)`.
  - `DragEnded` → `Commit((float)ScaleSlider.Value)`.
